...
namespace TimeDataViewerLite.RenderContext;

public class CanvasRenderContext
{
    private readonly int _maxFiguresPerGeometry = 16;
    private readonly int _maxPolylinesPerLine = 64;
    private readonly int _minPointsPerPolyline = 16;
    private readonly Dictionary<Color, IBrush> _abrushCache = new();
    ...
    private IBrush? GetCachedBrush(Color color)
    {
        ...
        if (!_abrushCache.TryGetValue(color, out var brush))
        {
    ...
    private void SetStroke(Shape shape, Color stroke, IEnumerable<double>? dashArray = null, double dashOffset = 0, bool aliased = false)
    {
        ...
        if (dashArray != null)
        {
            shape.StrokeDashArray = new Avalonia.Collections.AvaloniaList<double>(dashArray);
